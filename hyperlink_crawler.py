...
def save_urls_to_csv(year):
    df = pd.read_csv(f"./output/table/table_{year+1911}.csv", encoding='utf-8')
    df_urls = pd.read_csv(f"./output/table/url_{year+1911}.csv", encoding='utf-8')
    print(f"len of table: {len(df)}")
    print(f"len of df_urls: {len(df_urls)}")
    df = pd.concat([df, df_urls], axis=1, ignore_index=True)
    df.to_csv(f"./output/table/table_{year+1911}.csv", encoding='utf-8', index=False)
...
def run(year, selected_columns, len_columns):
    ...
    df_urls.to_csv(f"./output/table/url_{year+1911}.csv", encoding='utf-8', index=False)
